...
function toHtml($xetable, opts, columns, datas) {
  ...
  if (isHeader) {
    body += "<thead><tr>".concat(columns.map(function (column) {
      ...
      return "<th class=\"".concat(classNames.join(' '), "\" title=\"").concat(cellTitle, "\"><div ").concat(isPrint ? '' : "style=\"width: ".concat(column.renderWidth, "px\""), "><span>").concat(_tools.UtilTools.formatText(cellTitle, true), "</span></div></th>");
    }).join(''), "</tr></thead>");
  }
    ...
    if (treeConfig) {
      datas.forEach(function (item) {
        body += '<tr>' + columns.map(function (column) {
          ...
          return "<td class=\"".concat(classNames.join(' '), "\" title=\"").concat(cellValue, "\"><div ").concat(isPrint ? '' : "style=\"width: ".concat(column.renderWidth, "px\""), ">").concat(_tools.UtilTools.formatText(cellValue, true), "</div></td>");
        }).join('') + '</tr>';
      });
    } else {
      datas.forEach(function (item) {
...
function saveLocalFile(options) {
  ...
  if (window.Blob) {
    var blob = content instanceof Blob ? content : getExportBlobByContent(_ctor.default.toString(content), options);
...
function handleImport($xetable, content, opts) {
  ...
  if (status) {
    ...
    if (opts.message !== false) {
      _vXETable.default.modal.message({
        message: _conf.default.i18n('vxe.table.impSuccess', [rows.length]),
        status: 'success'
      });
    }
...
var _default = {
  methods: {
    ...
    _exportData: function _exportData(options) {
      ...
      if (!opts.filename) {
        opts.filename = _conf.default.i18n(opts.original ? 'vxe.table.expOriginFilename' : 'vxe.table.expFilename', [_ctor.default.toDateString(Date.now(), 'yyyyMMddHHmmss')]);
      }
    ...
    _importData: function _importData(options) {
      ...
      var opts = Object.assign({
        types: _vXETable.default.importTypes
      ...
      fileInput.onchange = function (evnt) {
        ...
        if (!isAllType) {
          for (var fIndex = 0; fIndex < files.length; fIndex++) {
            ...
            if (!_ctor.default.includes(types, type)) {
              errType = type;
              break;
            }
          }
        ...
        if (!errType) {
          ...
          if (options.message !== false) {
            _vXETable.default.modal.message({
              message: _ctor.default.template(_conf.default.i18n('vxe.error.notType'), [errType]),
              status: 'error'
            });
          }
