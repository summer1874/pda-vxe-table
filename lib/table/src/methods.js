...
var Methods = {
  ...
  loadColumn: function loadColumn(columns) {
    ...
    this.collectColumn = _ctor.default.mapTree(columns, function (column) {
  ...
  createData: function createData(records) {
    ...
    var rows = records.map(function (record) {
      return _this7.defineField(Object.assign({}, record, _defineProperty({}, rowkey, null)));
    });
    return this.$nextTick().then(function () {
      return rows;
    });
  ...
  createRow: function createRow(records) {
    ...
    return this.$nextTick().then(function () {
      return _this8.createData(records).then(function (rows) {
    ...
    if (arguments.length) {
      ...
      rows.forEach(function (row) {
        if (!_this9.isInsertByRow(row)) {
          var rowIndex = _this9.getRowIndex(row);
    ...
    if (keepSource) {
      ...
      if (treeConfig) {
        ...
        var matchObj = _ctor.default.findTree(tableSourceData, function (item) {
          return rowid === getRowid(_this10, item);
        }, treeOpts);
  ...
  updateAfterFullData: function updateAfterFullData() {
    ...
    if (column && column.order) {
      ...
      var isRemote = _ctor.default.isBoolean(column.remoteSort) ? column.remoteSort : sortOpts.remote || remoteSort;
      ...
      if (!isRemote) {
        if (allSortMethod) {
          ...
          var rest = column.sortMethod ? tableData.sort(column.sortMethod) : _ctor.default.sortBy(tableData, column.sortBy || (column.formatter ? function (row) {
  ...
  mergeCustomColumn: function mergeCustomColumn(customColumns) {
    ...
    if (customColumns.length) {
      tableFullColumn.forEach(function (column) {
        // 在 v3.0 中废弃 prop
        var item = _ctor.default.find(customColumns, function (item) {
          return column.property && (item.field || item.prop) === column.property;
        });

        if (item) {
          if (_ctor.default.isNumber(item.resizeWidth)) {
            column.resizeWidth = item.resizeWidth;
          }

          if (_ctor.default.isBoolean(item.visible)) {
  ...
  refreshColumn: function refreshColumn() {
    ...
    return this.$nextTick().then(function () {
      ...
      _this13.updateCellAreas();

      return _this13.$nextTick().then(function () {
        ...
        if (layout === 'header') {
          ...
          if (isGroup && listElem) {
            ...
            _ctor.default.arrayEach(listElem.querySelectorAll('.col--group'), function (thElem) {
              var colNode = _this16.getColumnNode(thElem);
        ...
        if (validTip && getEventTargetNode(evnt, validTip.$el).flag) {// 如果是激活状态，且点击了校验提示框
          ...
          if (!getEventTargetNode(evnt, document.body, 'vxe-table--ignore-clear').flag) {
            ...
            this.preventEvent(evnt, 'event.clearActived', actived.args, function () {
              ...
              if (!isClear && _this18.height && !_this18.overflowY) {
      ...
      if (!getEventTargetNode(evnt, $el).flag && !getEventTargetNode(evnt, $refs.tableWrapper).flag) {
        ...
        if (!getEventTargetNode(evnt, document.body, 'vxe-table--ignore-areas-clear').flag) {
          this.preventEvent(evnt, 'event.clearAreas', {}, function () {
            _this18.clearCellAreas();

            _this18.clearCopyCellArea();
          });
        }
      }
    ...
    if (this.isActivated) {
      this.preventEvent(evnt, 'event.keydown', null, function () {
        var isCtxMenu = _this19.isCtxMenu,
            ...
            bodyCtxMenu = _this19.bodyCtxMenu;
        ...
        if (operCtxMenu) {
        ...
        } else if (keyboardConfig && _this19.mouseConfig && _this19.mouseOpts.area && _this19.handleKeyboardEvent) {
          _this19.handleKeyboardEvent(evnt);
          ...
          if (selected.column.type === 'checkbox' || selected.column.type === 'selection') {
            _this19.handleToggleCheckRowEvent(evnt, selected.args);
          } else {
          ...
          // 如果按下上下文键
          _this19._keyCtx = selected.row && selected.column && bodyCtxMenu.length;
          clearTimeout(_this19.keyCtxTimeout);
          _this19.keyCtxTimeout = setTimeout(function () {
            _this19._keyCtx = false;
          }, 1000);
          ...
          if (!isEditStatus) {
            ...
            if (keyboardConfig.isDel && (selected.row || selected.column)) {
              ...
              // 如果树形表格回退键关闭当前行返回父节点
              var _XEUtils$findTree = _ctor.default.findTree(_this19.afterFullData, function (item) {
          ...
          if (!isEditStatus) {
            // 如果开启复制功能
            if (keyboardConfig.isCut && _this19.mouseConfig && _this19.mouseOpts.checked) {
      ...
      if (reserve) {
        if (value) {
          selectRows.forEach(function (row) {
            checkboxReserveRowMap[getRowid(_this24, row)] = row;
          });
        } else {
          afterFullData.forEach(function (row) {
            return _this24.handleCheckboxReserveRow(row, false);
          });
        }
      }
  ...
  handleAsyncRowExpand: function handleAsyncRowExpand(row) {
    ...
    return new Promise(function (resolve) {
      ...
      _this29.expandOpts.loadMethod({
        ...
        $rowIndex: _this29.getVMRowIndex(row)
      }).catch(function (e) {
        return e;
      }).then(function () {
        rest.expandLoaded = true;

        _ctor.default.remove(_this29.expandLazyLoadeds, function (item) {
          return item === row;
        });
    ...
    if (rows) {
      ...
      var validRows = toggleMethod ? rows.filter(function (row) {
        return toggleMethod({
          ...
          $rowIndex: _this30.getVMRowIndex(row)
        });
      }) : rows;
      ...
      if (reserve) {
        validRows.forEach(function (row) {
          return _this30.handleRowExpandReserve(row, expanded);
        });
      }
    }
  ...
  clearRowExpand: function clearRowExpand() {
    ...
    if (reserve) {
      tableFullData.forEach(function (row) {
        return _this31.handleRowExpandReserve(row, false);
      });
    }
  ...
  setTreeExpand: function setTreeExpand(rows, expanded) {
    ...
    if (rows) {
      ...
      if (rows.length) {
        ...
        if (reserve) {
          validRows.forEach(function (row) {
            return _this34.handleTreeExpandReserve(row, expanded);
          });
        }
  ...
  clearTreeExpand: function clearTreeExpand() {
    ...
    if (reserve) {
      _ctor.default.eachTree(tableFullData, function (row) {
        return _this35.handleTreeExpandReserve(row, false);
      }, treeOpts);
    }
    ...
    return this.$nextTick().then(function () {
      ...
      if (tableBodyElem) {
        ...
        if (scrollXLoad) {
          ...
          var visibleXSize = _ctor.default.toNumber(sXOpts.vSize);
        ...
        if (scrollYLoad) {
          ...
          var visibleYSize = _ctor.default.toNumber(sYOpts.vSize || Math.ceil(tableBodyElem.clientHeight / rHeight));
    ...
    return this.$nextTick().then(function () {
      ...
      if (scope && $refs.tableBody && editRules) {
        ...
        if (_this40.hasCellRules(type, row, column)) {
          var cell = _this40.getCell(row, column);
  ...
  setMergeCells: function setMergeCells(merges) {
    ...
    return this.$nextTick().then(function () {
      return _this41.updateCellAreas();
    });
  },
  ...
  setMergeFooterItems: function setMergeFooterItems(merges) {
    ...
    return this.$nextTick().then(function () {
      return _this43.updateCellAreas();
    });
  },
